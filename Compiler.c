...
static int variable()
{
	...
	int regi = next_register();

	if(is_identifier(token))
	{
		CodeGen(LOAD, regi, token, EMPTY_FIELD);
		next_token();
		return regi;
	}
	...
	{
		ERROR("Error in variable method, %c \n", token);
		exit(EXIT_FAILURE);
	}
}
...
static void assign()
{
	...
	if (token == '=')
	{
		next_token();
		reg = expr();
		CodeGen(STORE, v, reg, EMPTY_FIELD);
	}
...
static void swap()
{
	...
	char t2 = token;
	int reg2= variable();
	CodeGen(STORE, t, reg2, EMPTY_FIELD);
	CodeGen(STORE, t2, reg, EMPTY_FIELD); 
}
